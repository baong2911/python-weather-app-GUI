...
def getWeather():
    ...
    try:
        ...
        for entry in json_forecast_data['list']:
            ...
            # If the day is not in the dictionary, add it with the current temperatures and icon URL
            if day not in daily_info:
                daily_info[day] = {'min': temp_min, 'max': temp_max, 'icon_url': icon_url}
            else:
                # Update the temperatures and icon URL if a lower min or higher max is encountered
...
def update_forecast_labels():
    ...
    for i, (day, info) in enumerate(daily_info.items()):
        # Update the day label with weather information
        day_labels[i]['text'] = f"{day}\nMin Temp: {info['min']}°C\n Max Temp: {info['max']}°C"

        # Update the image in the corresponding day box
...
#7 days forecast 
days_text=PhotoImage(file="5days_text.png")
text_7days=Label(image=days_text)
text_7days.place(x=600,y=50)

# Right box
frame = Frame(root, width=200, height=750, bg="#212120")
frame.pack(side=RIGHT)

firstbox = PhotoImage(file="rounded.png")
resized_firstbox = firstbox.subsample(3, 3)
resized_firstbox_2 = firstbox.subsample(3, 5)

# Label(frame, image=resized_firstbox, bg="#212120").place(x=15, y=20)
# Label(frame, image=resized_firstbox_2, bg="#212120").place(x=15, y=150)
# Label(frame, image=resized_firstbox_2, bg="#212120").place(x=15, y=230)
# Label(frame, image=resized_firstbox_2, bg="#212120").place(x=15, y=310)
# Label(frame, image=resized_firstbox_2, bg="#212120").place(x=15, y=390)
# Label(frame, image=resized_firstbox_2, bg="#212120").place(x=15, y=470)
